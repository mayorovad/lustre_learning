...
>Действия, которые выполняются в данном отчете следует выполнять после сборки модифицированного ядра, либо после скачивания последней версии, собранной в whamcloud.
Отчет о том, как собрать модифицированное ядро, находится [здесь](./patching_kernel.md). Собирать не из-под рута, устанавливать из-под рута
...
```
Установка пакетов для сервера также позволяет использовать на машине все функции клиента.
...
Установка аналогична установке Lustre для сервера, однако пакеты клиента не требуют установки модифицированного ядра (но требуют ту же версию ядра).
